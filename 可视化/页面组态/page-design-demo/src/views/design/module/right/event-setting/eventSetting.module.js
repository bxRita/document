/*
 * FilePath: \src\views\design\module\right\event-setting\eventSetting.module.js
 * Project: page-design-demo
 * CreatedAt: 2021-04-14 16:34:46
...
export default class EventSetting {
  ...
  createEventListener() {
    ...
    this.eventData = Object.keys(this.eventListener).forEach(item => {
      let eventItem = {
        eventName: item,
        eventCallBack: this.eventListener[item]
      }
      item.indexOf('$$') < 0
        ? eventData.push(eventItem)
        : linkageEventData.push(eventItem)
    })
  ...
  createEventFunction(item, functionName) {
    const eventHandlerFun = `
        return function ${
          functionName
            ? functionName
            : 'on' + firstUpperCase(item.eventName) + 'EventCallBack'
        }(currentComponentInstance, ...arg) {
          /**
    ...
    return Object.assign({}, item, {
      eventCallBack: new Function(eventHandlerFun)()
    })
  }
  /**
   * 对比事件绑定函数是否有修改
  ...
  diffEventListenerCode(codeValue, eventName) {
    if (codeValue && this.eventListener[eventName]) {
      if (
        codeValue.toString().replace(/\s/g, '') !==
        this.eventListener[eventName].toString().replace(/\s/g, '')
      ) {
        return true
      }
    } else if (codeValue) {
  ...
  delEventListener(eventName) {
    ...
    if (Object.prototype.hasOwnProperty.call(eventListener, eventName)) {
  ...
  updateEventListener(key, value) {
    this.eventListener = Object.assign(
      {},
      this.eventListener,
      arguments.length >= 2 ? { [key]: value } : key
    )
  ...
  setLinkageEventLevel(data, index) {
    ...
    if (index === 2 && currentEventListener) {
      return Object.assign(
        {},
        {
          eventName: currentLinkageEventKey,
          eventCallBack: currentEventListener
        }
      )
    }
    ...
    // 选择联动类型 新增函数
    if (index === 2) {
      return this.createEventFunction(
        { eventName: `${this.linkageEventLevel.join('$$')}` },
        data
      )
    }
  ...
  findLinkageEvent() {
    ...
    keys = Object.keys(this.eventListener).filter(
      item => item.indexOf('$$') > -1
    )
